...
class TemporalReasoner():
    ...
    def __init__(self):
        ...
        self.start_time = time.time()
        self.relative_path = os.path.join(os.getcwd(), "..", "/output/run_output/")
    ...
    def save_video(self, frames_list, params):
        ...
        video_mp4_path = os.getcwd() + self.relative_path + "input_video.mp4"
        video_avi_path = os.getcwd() + self.relative_path + "input_video.avi"
    ...
    def usage(self, user_prompt, params, prompts):
        ...
        output_textfile_path = os.getcwd() + self.relative_path + "output.txt"
        output_image_path = os.getcwd() + self.relative_path + "output.png"
        
        
        parser_output = eval(parser.to_llm(user_prompt, params, prompts))
        modified_prompt = parser_output["past"]

        if os.path.exists(output_textfile_path):
            os.remove(output_textfile_path, 'output.txt')
        ...
        videoLLM_output = validator.conversation(params, modified_prompt, self.input_list[-1], prompts, output_textfile_path)
        cogvlm_query = eval(parser.to_cogVLM(videoLLM_output, params, prompts))
        cogvlm_query = cogvlm_query['output']
        bbox_coords = grounder.send_msg_to_server(self.input_list[-1],cogvlm_query, output_image_path)
